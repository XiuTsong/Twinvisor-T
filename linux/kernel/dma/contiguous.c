...
#if 0
    pr_info("%s: reserving %ld MiB for secure_memory_area\n", __func__,
            (unsigned long)selected_size / SZ_1M);
    pr_info("%s: init secure_memory_area return %d\n", __func__,
            dma_contiguous_reserve_area(selected_size, selected_base,
                memblock_end_of_DRAM(), &secure_memory_area, fixed));
    secure_memory_dev.cma_area = secure_memory_area;
    pr_info("%s: secure_memory_dev = %px, cma_area = %px\n", __func__,
            &secure_memory_dev, secure_memory_dev.cma_area);
#else
