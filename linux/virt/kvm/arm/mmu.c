			     ...
			     void *data)
{
    ...
    if (is_migrating) {
        ...
        } else if (migrate_sec_vm_id != kvm->arch.sec_vm_id) {
            pr_err("%s: ERROR multi-sec-VM %d:%d not support now\n",
                    __func__, migrate_sec_vm_id, kvm->arch.sec_vm_id);
            asm volatile("b .\n\t");
        }
    }
	...
	kvm_for_each_memslot(memslot, slots) {
        ...
        if (is_migrating) {
            /* Batch IPNs here */
            if ((gpa >> PAGE_SHIFT) == 0x404ca)
                pr_err("%s:%d GPA %lx is going to be unmapped\n",
