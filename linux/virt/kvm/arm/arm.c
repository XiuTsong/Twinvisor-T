...
{
	return (shared_register_pages + smp_processor_id() * TITANIUM_MAX_SIZE_PER_CORE);
}

inline void *get_gp_reg_region(unsigned int core_id) {
    uint64_t *ptr = shared_register_pages + core_id * TITANIUM_MAX_SIZE_PER_CORE;
    return (void *)ptr;
}
...
static void prepare_el2_regs(kvm_smc_req_t *smc_req)
{
	u64 vttbr_el2, vmpidr_el2, vpidr_el2;

	vttbr_el2 = read_sysreg(vttbr_el2);
	vmpidr_el2 = read_sysreg(vmpidr_el2);
	vpidr_el2 = read_sysreg(vpidr_el2);

	smc_req->el2_regs.vttbr_el2 = vttbr_el2;
	smc_req->el2_regs.vmpidr_el2 = vmpidr_el2;
	smc_req->el2_regs.vpidr_el2 = vpidr_el2;
}

static void lr_list_append(uint64_t *lr_list, uint64_t val)
{
	u32 i;
...
static void kvm_smc_req_append_lrs(struct kvm_vcpu *vcpu, kvm_smc_req_t *smc_req, int core_id)
{
	u32 i;
	u64 *titanium_ich_lr = vcpu->titanium_ich_lr;

	for (i = 0U; i < VGIC_V3_MAX_LRS; ++i) {
		if (titanium_ich_lr[i] != 0) {
			lr_list_append(smc_req->icc_ich_lr, titanium_ich_lr[i]);
		}
	}
	memset(titanium_ich_lr, 0, sizeof(u64) * VGIC_V3_MAX_LRS);
}

void trap_titanium_enter_guest(struct kvm_vcpu *vcpu)
{
	u32 sec_vm_id = vcpu->kvm->arch.sec_vm_id;
	int core_id = smp_processor_id();
    kvm_smc_req_t* smc_req = get_smc_req_region(core_id);

    smc_req->sec_vm_id = sec_vm_id;
	prepare_el2_regs(smc_req);
	kvm_smc_req_append_lrs(vcpu, smc_req, core_id);
}
...
int kvm_arch_init_vm(struct kvm *kvm, unsigned long type)
{
    ...
    boot_titanium_secure_vm(kvm->arch.sec_vm_id);
...
void kvm_arch_destroy_vm(struct kvm *kvm)
{
    ...
    destroy_titanium_secure_vm(kvm->arch.sec_vm_id);
...
/* Bind vcpu to current core for simplicity */
static void kvm_vcpu_init_bind_core(struct kvm_vcpu *vcpu)
{
	int core_id = smp_processor_id();
	struct task_struct *tsk = current;

	if (tsk == NULL) {
		pr_info("xxxxx: current task is NULL\n");
		return;
	}
	if (core_id != vcpu->cpu) {
		pr_info("mismatch core_id: %d, vcpu->cpu: %d\n", core_id, vcpu->cpu);
	}
	cpumask_clear(&tsk->cpus_allowed);
	cpumask_set_cpu(core_id, &tsk->cpus_allowed);
	pr_info("task: %s pid: %d bind vcpu to core: %d\n", tsk->comm, tsk->pid, core_id);
	vcpu->bind_core = true;
}
...
int kvm_arch_init(void *opaque)
{
    ...
    kvm_info("Register shared memory started.");
    register_titanium_shared_memory();
    kvm_info("Register shared memory succeeded.");
