...
static u_register_t __el3_data hcr_el2[TITANIUM_CORE_COUNT];
 ...
 ******************************************************************************/
static uintptr_t __el3_text titanium_smc_handler(uint32_t smc_fid,
			 u_register_t x1,
			 u_register_t x2,
			 u_register_t x3,
			 u_register_t x4,
			 ...
			 u_register_t flags)
{
	 ...
	 */
	smc_imm = read_sysreg(esr_el2) & 0xffff;
	if (smc_imm == 0) {
		el3_panic();
	}
	...
	if (is_caller_non_secure(flags)) {
		...
		hcr_el2[linear_id] = read_sysreg(hcr_el2);
		cm_el1_sysregs_context_save(NON_SECURE);
		cm_el2_sysregs_context_save(NON_SECURE, true);

		/* hvc will change the value of ELR_EL2, SPSR_EL2, ESR_EL2
		* We restore the original value from n-visor of this registers here
		*/
		cm_el2_sysregs_context_restore(NON_SECURE, true);
		...
		cm_el2_sysregs_context_restore(SECURE, true);
		...
		/* Only reserve HCR_EL2.E2H and HCR_EL2.RW */
	...
	cm_el2_sysregs_context_save(SECURE, true);
	...
	write_sysreg(hcr_el2[linear_id], hcr_el2);
