...
static vm_fault_t wp_page_copy(struct vm_fault *vmf)
{
	...
	if (likely(pte_same(*vmf->pte, vmf->orig_pte))) {
        ...
        if (new_page->is_sec_mem) {
            pr_err("%s:%d PFN: %lx from alloc_pages\n",
                    __func__, __LINE__, page_to_pfn(new_page));
            asm volatile("b .");
        }
		/*
...
#if 0
        sec_page_cnt++;
        if (0x3000000 + sec_page_cnt - 1 != page_to_pfn(page))
            printk("\t !!!: %s: [%llu -> %llx] pfn = %lx, vmf->address = %lx\n", 
                    __func__, sec_page_cnt, 0x3000000 + sec_page_cnt - 1,
                    page_to_pfn(page), vmf->address);
#endif
    } else {
