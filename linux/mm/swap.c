...
void __put_page(struct page *page)
{
	...
	if (unlikely(PageCompound(page)))
        ...
        pr_err("%s: !!!!! put secure memory page, pfn: %lx, count: %d:%d\n",
                __func__, page_to_pfn(page), page_count(page), page_ref_count(page));
        BUG();
    }
	else
...
void release_pages(struct page **pages, int nr)
{
	...
	for (i = 0; i < nr; i++) {
        ...
        if (is_sec_vm && page->is_sec_mem) {
            if (PageLRU(page)) {
                pr_err("%s:%d ERROR SMA PFN %lx should NOT be LRU\n",
                        __func__, __LINE__, page_to_pfn(page));
            }
            list_add(&page->lru, &sec_mem_pages);
            continue;
        } else if (page->is_sec_mem) {
            pr_err("%s:%d ERROR release SMA PFN %lx when S-VM is NOT exiting\n",
                    __func__, __LINE__, page_to_pfn(page));
            BUG();
        }
    ...
    if (is_sec_vm) {
        ...
        list_for_each_entry_safe(page_it, next, &sec_mem_pages, lru) {
            if (page_it->is_sec_mem) {
                ...
                pr_err("%s:%d ERROR: non-secure page %lx in sec_mem_pages\n",
                        __func__, __LINE__, page_to_pfn(page_it));
            }
        }
    }
